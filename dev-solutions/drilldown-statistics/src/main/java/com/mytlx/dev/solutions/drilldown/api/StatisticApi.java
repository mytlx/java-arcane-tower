...
import com.mytlx.dev.solutions.drilldown.service.statistic.DashboardService;
import com.mytlx.dev.solutions.drilldown.service.statistic.StatisticAggDetailService;
...
public class StatisticApi {
    ...
    @Autowired
    private StatisticAggDetailService aggDetailService;
    ...
    // @Operation(value = "聚合统计列表页", result = MeiShiStatisticAggVO.class)
    // @ApiParams({
    ...
    public Out aggList(In in) throws Exception {
        return aggListService.platformAggList(in);
    }
    ...
    //         @ApiParam(value = "showType", description = "点击数据行showType，StatisticAggVO.AggItem#showType"),
    // })
    public Out aggDetail(In in) throws Exception {
        return aggDetailService.aggDetail(in);
    }

    // @Api(value = "聚合统计列表页", result =StatisticAggVO.class)
    // @ApiParams({
