...
import com.mytlx.arcane.utils.json.fastjson.Fastjson2Utils;
...
/**
 * Fastjson2 实现的 JSON 处理类
 * <p>
 * 该类是 {@link JsonHandler} 接口的 Fastjson2 实现，提供了基于 Fastjson2 库的 JSON 序列化和反序列化功能。
 ...
 * @since 2025-09-19 11:50:37
 */
public class Fastjson2Handler implements JsonHandler {
    /**
     * 将对象序列化为 JSON 字符串
     * <p>
    ...
    public String toJson(Object object) {
        return Fastjson2Utils.toJson(object);
    }
     ...
     */
    @Override
    public String toJsonOrThrow(Object object) throws JSONException {
        return Fastjson2Utils.toJsonOrThrow(object);
    }
    ...
    public <T> T fromJson(String json, Class<T> clazz) {
        return Fastjson2Utils.fromJson(json, clazz);
    }
     ...
     * @throws JSONException            如果反序列化过程中发生错误
     * @see #fromJson(String, Class)
     */
    @Override
    public <T> T fromJsonOrThrow(String json, Class<T> clazz) throws JSONException {
        return Fastjson2Utils.fromJsonOrThrow(json, clazz);
    }
     ...
     * @param <T>  目标类型
     * @param json JSON 字符串，可以为 {@code null}
     * @param type 目标类型的 {@link Type} 对象，通常通过 {@link com.alibaba.fastjson2.TypeReference} 获取
     * @return 反序列化后的对象，如果 JSON 为 {@code null}、空或反序列化失败则返回 {@code null}
     * @see #fromJsonOrThrow(String, Type)
     */
    @Override
    public <T> T fromJson(String json, Type type) {
        return Fastjson2Utils.fromJson(json, type);
    }
     ...
     */
    @Override
    public <T> T fromJsonOrThrow(String json, Type type) throws JSONException {
        return Fastjson2Utils.fromJsonOrThrow(json, type);
    }
    ...
    public <E> List<E> toList(String json, Class<E> clazz) {
        return Fastjson2Utils.toList(json, clazz);
    }
     ...
     */
    @Override
    public <E> List<E> toListOrThrow(String json, Class<E> clazz) throws JSONException {
        return Fastjson2Utils.toListOrThrow(json, clazz);
    }
     ...
     * @param <K>        映射键类型
     * @param <V>        映射值类型
     * @param json       JSON 对象字符串，例如 {@code {"1":{"name":"Alice"},"2":{"name":"Bob"}}}
    ...
    public <K, V> Map<K, V> toMap(String json, Class<K> keyClass, Class<V> valueClass) {
        return Fastjson2Utils.toMap(json, keyClass, valueClass);
    }
     ...
     */
    @Override
    public <K, V> Map<K, V> toMapOrThrow(String json, Class<K> keyClass, Class<V> valueClass) throws JSONException {
        return Fastjson2Utils.toMapOrThrow(json, keyClass, valueClass);
    }
    ...
    public String toPrettyJson(Object object) {
        return Fastjson2Utils.toPrettyJson(object);
    }
    ...
    public boolean isValidJson(String json) {
        return Fastjson2Utils.isValidJson(json);
    }
}